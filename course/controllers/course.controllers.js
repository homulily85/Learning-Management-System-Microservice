...
export const getAllCourse = asyncHandler(async (req, res, next) => {
    try {
        ...
        // --- CACHE LOGIC START ---
        // Only cache if it is one of the first 10 pages AND limit is standard
        // (10)
        const isCacheable = page <= 10 && limit === 10
        const cacheKey = `courses:page:${page}:limit:${limit}`

        if (isCacheable) {
            const cachedData = await redis.get(cacheKey)
            if (cachedData) {
                // CACHE HIT: Return parsed JSON immediately
                return res.status(200).json({
                    success: true,
                    message: 'All courses fetched successfully (from cache)',
                    ...JSON.parse(cachedData), // Spread the cached courses &
                                              // pagination info
                })
            }
        }
        ...
        if (isCacheable) {
            ...
            // JSON.stringify is needed because Redis stores strings
...
/**
 * @REMOVE_LECTURE
 * Removes a lecture from a course by its ID and deletes the video from
 *     Cloudinary.
 */
